...
const Profile: React.FC = () => {
	...
	const fetchCartItems = async () => {
		try {
			const response: ApiResponse<Cart> = await getCart();
	...
	const fetchWishlistItems = async () => {
		try {
			...
			console.error("Erreur lors de la récupération de la liste d'envies");
		}
	};
	...
	const handleLogout = () => {
		if (
			window.confirm(
				"Vous êtes déconnecté. Vous allez être redirigé vers la page d'accueil.",
			)
		) {
